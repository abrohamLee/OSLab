...
// Signal handler for SIGTERM (optional, for graceful termination)
...
int main(int argc, char *argv[]) {
    ...
    int prompts_received = 0;
    char prompt_buffer[MAX_PROMPT_LENGTH];

    while (prompts_received < MAX_PROMPTS && !terminate && fgets(prompt_buffer, sizeof(prompt_buffer), stdin) != NULL) {
        ...
        printf("user\n %s \n", prompt_buffer);
        fflush(stdout); // Ensure the user prompt is printed before generation

        generate(prompt_buffer);   

        // Send SIGUSR1 to parent process to indicate completion
        if (kill(parent_pid, SIGUSR1) == -1) {
            perror("kill SIGUSR1 to parent");
            // Optionally handle the error, e.g., exit if parent is not reachable
            if (errno == ESRCH) {
                // Parent process does not exist
                fprintf(stderr, "Parent process does not exist. Exiting.\n");
                break;
            }
        }
